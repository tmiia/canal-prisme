...
export default function CursorFollower() {
  ...
  useEffect(() => {
    const onPointerMove = (e: PointerEvent) => {
      if (spanRef.current) {
        spanRef.current.style.transform = `translate(${e.clientX + 12}px, ${e.clientY + 12}px)`;
      }
    };
  ...
  return (
    <span
      ref={spanRef}
      className="fixed top-0 left-0 pointer-events-none bg-black text-white text-xs p-1 px-1.5 z-50 font-canal-light-italic hidden md:block transition-transform duration-100 ease-out"
    >
